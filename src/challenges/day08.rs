...
fn decrypt(input: &str) -> Option<[&str; 10]> {
    let mut ret = [""; 10];
    let strings = input.split_whitespace().collect::<Vec<_>>();

    /* Unique char count */
    ret[1] = strings.iter().find(|&&s| s.len() == 2)?;
    ret[7] = strings.iter().find(|&&s| s.len() == 3)?;
    ret[4] = strings.iter().find(|&&s| s.len() == 4)?;
    ret[8] = strings.iter().find(|&&s| s.len() == 7)?;

    /* 5 characters */
    ret[5] = strings.iter().find(|&&s| {
        s.len() == 5
            && ret[4]
                .chars()
    ...
    let middle_segment = ret[4]
        .chars()
        .find(|&c| !ret[1].contains(c) && ret[2].contains(c))?;
    let top_right_segment = ret[1].chars().find(|&c| ret[2].contains(c))?;
...
fn part_2() -> usize {
    ...
    for (input, output) in data.iter().filter_map(|s| s.split_once('|')) {
        let decrypt_map = decrypt(input).expect("Could not decrypt input!");
        let mut res = 0;
