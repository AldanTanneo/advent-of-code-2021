...
//! [hydrothermal vents](https://en.wikipedia.org/wiki/Hydrothermal_vent) on
...
const NAME: &str = "Hydrothermal Venture";
...
impl FromStr for Line {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let mut from = split.next().expect("Ill-formatted line!").split(',');
        let mut to = split.next().expect("Ill-formatted line!").split(',');

        let mut res = Self {
            from: Point {
                x: from.next().expect("Ill-formatted line!").parse()?,
                y: from.next().expect("Ill-formatted line!").parse()?,
            },
            to: Point {
                x: to.next().expect("Ill-formatted line!").parse()?,
                y: to.next().expect("Ill-formatted line!").parse()?,
            },
        };
        if res.from.y > res.to.y {
            std::mem::swap(&mut res.from, &mut res.to);
        }
        Ok(res)
    }
}
    ...
    for Line { from, to } in data {
        if from.x == to.x {
            ...
            for i in 0..=(to.x - from.x) {
                grid[from.x + i + 1000 * (from.y + i)] += 1;
            }
        } else if from.x > to.x {
            for i in 0..=(from.x - to.x) {
                grid[from.x - i + 1000 * (from.y + i)] += 1;
            }
        }
    }
